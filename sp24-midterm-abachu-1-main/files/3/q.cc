...
float Calculator::Calculate(int a, int b, Operation operation){
    float output;   //storing result in output
    switch(operation){
        case Operation::kAdd:
            output = a+b;
            break;
        ...
        case Operation::kDivision:
            if (b==0){
                ...
                output=0;
                history.push_back({a,b,operation, output});      //storing the value though it is not divisible ny 0 with the given requirements
                return 0;
    ...
    history.push_back({a,b,operation, output});   //storing result in history
    return output;
}
float Calculator::Calculate(int a, Operation operation){
    int b=0;
    if (!history.empty()) b=history.back().result;    //storing history
